...
function sensorKataKotor(text: string) {
  ...
  kataKotor.forEach(kata => {
    // Ganti semua kemunculan kata kotor di mana saja dalam string (tanpa boundary)
                  ...
                  {reviews.slice(0, 5).map((review: any) => (
                    <div key={review.id} className="border-b border-slate-200 pb-6 last:border-0 hover:bg-slate-50 p-4 rounded-lg transition-colors duration-200">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="font-medium text-slate-900">{isAdmin ? review.nama : sensorKataKotor(review.nama)}</h3>
                      ...
                      <p className="text-slate-600 mt-2">{isAdmin ? review.komentar : sensorKataKotor(review.komentar)}</p>
                    </div>
                  ))}
                </div>
              </div>
